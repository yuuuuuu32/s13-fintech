...
const getPosition = (index: number): [number, number, number] => {
  const TILES_PER_SIDE = 8; // 한 변의 타일 수 (코너 포함)
  const TILE_WIDTH = 3;
  ...
  const side = Math.floor(index / TILES_PER_SIDE); // 0: 아래, 1: 왼쪽, 2: 위, 3: 오른쪽
...
const getTextRotationY = (index: number): number => {
    ...
    switch (side) {
        case 0: return 0; // 아래
        case 1: return Math.PI / 2; // 왼쪽
        case 2: return Math.PI; // 위
        case 3: return -Math.PI / 2; // 오른쪽
        default: return 0;
    }
}
