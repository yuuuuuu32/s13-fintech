...
import {
  boardData as initialBoardData,
  BuildingType,
} from "../data/boardData.ts";
...
const chanceCards = [
  ...
  },
  {
    ...
    action: (player: Player) => ({
      ...player,
      position:
        (player.position - 3 + initialBoardData.length) %
        initialBoardData.length,
    }),
  },
  {
...
const handleInsufficientFundsForToll = (
  set: (partial: Partial<GameState> | ((state: GameState) => Partial<GameState>)) => void,
  get: () => GameState,
  requiredAmount: number,
  propertiesToSell: { index: number; price: number }[],
...
export const useGameStore = create<GameState>()((set, get) => ({
  ...
  initializeGame: (initialState: GameInitialState) => {
    ...
    const mappedState = {
      ...
      board: initialState.currentMap.cells.map(
        (cell: TileData | null) => cell || { name: "빈칸", type: "special" as const }
      ),
      players: playersArray,
      currentPlayerIndex: initialState.currentPlayerIndex,
      gamePhase: "SELECTING_ORDER" as GamePhase,
    };
    set(mappedState);
  ...
  buyProperty: () => {
    set((state) => {
      ...
      return {
        modal: { type: "INFO", text: "자산이 부족하여 구매할 수 없습니다." },
      };
    });
  },
  ...
  acquireProperty: () => {
    set((state) => {
      ...
      return {
        modal: { type: "INFO", text: "자산이 부족하여 인수할 수 없습니다." },
      };
    });
  },
