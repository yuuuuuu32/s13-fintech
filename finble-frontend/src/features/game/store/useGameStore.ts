...
interface GameState {
  ...
  checkGameOver: () => void
  handleJail: () => void
  handleInsufficientFundsForToll: (requiredAmount: number, propertiesToSell: { index: number; price: number; }[], currentPlayer: Player, players: Player[], currentPlayerIndex: number, tileIndex: number, toll: number) => { players: Player[]; modal: { type: string; text: string; }; } | undefined;
  payBail: () => void
...
const handleCityCompanyTile = (set: any, get: any, currentTile: TileData, currentPlayer: Player, players: Player[]) => {
...
const handleChanceTile = (set: any, get: any, currentTile: TileData, currentPlayer: Player, chanceCards: any[]) => {
...
const handleSpecialTile = (set: any, get: any, currentTile: TileData, currentPlayer: Player, board: TileData[]) => {
...
const handleInsufficientFundsForToll = (set: any, get: any, requiredAmount: number, propertiesToSell: { index: number; price: number; }[], currentPlayer: Player, players: Player[], currentPlayerIndex: number, tileIndex: number, toll: number) => {
  let moneyRaised = 0;
  ...
  payToll: () => {
    set(state => {
      ...
      if (currentPlayer.money < toll) {
          ...
          const result = handleInsufficientFundsForToll(set, get, requiredAmount, propertiesToSell, currentPlayer, players, currentPlayerIndex, tileIndex, toll);
